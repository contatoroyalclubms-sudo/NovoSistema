...
class UsuarioBase(BaseSchema):
    ...
    telefone: Optional[str] = Field(None, regex=r'^\(\d{2}\) \d{4,5}-\d{4}$', description="Telefone no formato (00) 00000-0000")
    ...
    cpf: Optional[str] = Field(None, regex=r'^\d{3}\.\d{3}\.\d{3}-\d{2}$', description="CPF no formato 000.000.000-00")
...
class UsuarioUpdate(BaseSchema):
    ...
    telefone: Optional[str] = Field(None, regex=r'^\(\d{2}\) \d{4,5}-\d{4}$')
...
class EventoBase(BaseSchema):
    ...
    cor_primaria: str = Field("#0EA5E9", regex=r'^#[0-9A-Fa-f]{6}$', description="Cor primária (hex)")
    cor_secundaria: str = Field("#64748B", regex=r'^#[0-9A-Fa-f]{6}$', description="Cor secundária (hex)")
...
class EventoUpdate(BaseSchema):
    ...
    cor_primaria: Optional[str] = Field(None, regex=r'^#[0-9A-Fa-f]{6}$')
    cor_secundaria: Optional[str] = Field(None, regex=r'^#[0-9A-Fa-f]{6}$')
...
class CategoriaBase(BaseModel):
    ...
    cor: str = Field(default="#007bff", regex=r"^#[0-9A-Fa-f]{6}$")
...
class CategoriaUpdate(BaseModel):
    ...
    descricao: Optional[str] = None
    cor: Optional[str] = Field(None, regex=r"^#[0-9A-Fa-f]{6}$")
    icone: Optional[str] = None
...
class MovimentoEstoqueBase(BaseModel):
    ...
    produto_id: str
    tipo_movimento: str = Field(..., regex=r"^(entrada|saida|ajuste)$")
...
class PromoterConquistaCreate(BaseModel):
    ...
    tipo: str = Field(..., regex=r"^(vendas|participacao|engajamento)$")
    nivel: str = Field(default="bronze", regex=r"^(bronze|prata|ouro|diamante)$")
...
class ContratanteEvento(BaseModel):
    ...
    tipo_pessoa: str = Field(..., regex=r'^(fisica|juridica)$')
