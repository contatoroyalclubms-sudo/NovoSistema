...
class RefundPriorityEnum(str, Enum):
    LOW = "low"
    MEDIUM = "medium"
    HIGH = "high"
    URGENT = "urgent"
...
class CheckinRequest(BaseSchema):
    ...
    @validator('qr_code')
    def pelo_menos_um_identificador(cls, v, values):
        if not v and not values.get('participante_id'):
...
class RefundRequest(BaseSchema):
    """Schema para solicitação de estorno"""
    transaction_id: str = Field(..., description="ID da transação original")
    amount: Decimal = Field(..., gt=0, description="Valor do estorno")
    reason: RefundReasonEnum = Field(..., description="Motivo do estorno")
    description: Optional[str] = Field(None, max_length=1000, description="Descrição adicional")
    priority: RefundPriorityEnum = Field(RefundPriorityEnum.MEDIUM, description="Prioridade da solicitação")
    customer_contact: Optional[EmailStr] = Field(None, description="Email de contato do cliente")

class RefundResponse(BaseSchema):
    """Schema de resposta para solicitação de estorno"""
    refund_id: str
...
class RefundStatusResponse(BaseSchema):
    """Schema de resposta para status de estorno"""
    refund_id: str
    status: RefundStatusEnum
    amount: Decimal
    created_at: datetime
...
class RefundApprovalRequest(BaseSchema):
    """Schema para aprovação de estorno"""
    notes: Optional[str] = Field(None, max_length=500, description="Notas da aprovação")

class RefundApprovalResponse(BaseSchema):
...
class RefundRejectionRequest(BaseSchema):
    """Schema para rejeição de estorno"""
    reason: str = Field(..., min_length=1, max_length=200, description="Motivo da rejeição")
    notes: Optional[str] = Field(None, max_length=500, description="Notas adicionais")

class RefundRejectionResponse(BaseSchema):
    """Schema de resposta para rejeição"""
    refund_id: str
    rejected: bool
    rejected_by: str
...
class RefundEligibilityRequest(BaseSchema):
...
class RefundEligibilityResponse(BaseSchema):
    """Schema de resposta para elegibilidade"""
    eligible: bool
...
class RefundSuggestionsRequest(BaseSchema):
    """Schema para solicitação de sugestões de IA"""
    reason: str
    category: Optional[str] = None
    transaction: Optional[Dict[str, Any]] = None

class RefundSuggestionsResponse(BaseSchema):
    """Schema de resposta para sugestões"""
    suggestions: List[str]
    context: str
    generated_at: datetime

class RefundMetrics(BaseSchema):
    """Schema para métricas de estorno"""
    total_refunds: int
    ...
    chargebacks_prevented: int

class RefundMetricsResponse(BaseSchema):
    """Schema de resposta para métricas"""
...
class RefundTrendData(BaseSchema):
...
class RefundTrendsResponse(BaseSchema):
    """Schema de resposta para tendências"""
    trends: List[RefundTrendData]
    time_range: str
    granularity: str
    generated_at: datetime

class RefundListResponse(BaseSchema):
    """Schema de resposta para listagem de estornos"""
    refunds: List[Dict[str, Any]]
    total_count: int
    limit: int
    offset: int

class ChargebackRiskRequest(BaseSchema):
    """Schema para análise de risco de chargeback"""
    transaction_id: str
    payment_data: Dict[str, Any]
    customer_data: Dict[str, Any]

class ChargebackRiskResponse(BaseSchema):
    """Schema de resposta para risco de chargeback"""
    transaction_id: str
    risk_score: float = Field(..., ge=0, le=1)
    risk_level: str
    risk_factors: List[str]
    recommendations: List[str]
    prevention_alert: Optional[Dict[str, Any]] = None
    analyzed_at: datetime

class ChargebackAnalyticsResponse(BaseSchema):
    """Schema de resposta para analytics de chargeback"""
    overview: Dict[str, Any]
    by_status: Dict[str, Any]
    by_category: Dict[str, Any]
    by_gateway: Dict[str, Any]
    trends: Dict[str, Any]
    prevention_metrics: Dict[str, Any]
    generated_at: datetime

class RefundQueueResponse(BaseSchema):
    """Schema de resposta para fila de aprovação"""
    pending_refunds: List[Dict[str, Any]]
    total_count: int
    priority_filter: Optional[RefundPriorityEnum] = None
    generated_at: datetime

class AdminRefundMetrics(BaseSchema):
    """Schema para métricas administrativas"""
    workflow_metrics: Dict[str, Any]
    chargeback_metrics: Dict[str, Any]
